...
model = UNet(in_channels=1, out_channels=13).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
...
def compute_surface_distances(pred, target, spacing):
    ...
    return distances_target_to_pred[pred_surface], distances_pred_to_target[target_surface]
...
train_model(model, train_loader, None, epochs=50)
